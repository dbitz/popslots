...
public class IabHelper {
    ...
    public void startSetup(final OnIabSetupFinishedListener listener) {
        ...
        this.mServiceConn = new ServiceConnection() {
            public void onServiceDisconnected(ComponentName name) {
                IabHelper.this.logInfo("Billing service disconnected.");
                Utils.reportAnalytics("sale", "debug", "purchaseServiceDisconnected", null);
                IabHelper.this.mSetupDone = false;
                IabHelper.this.mDisposed = false;
            ...
            public void onServiceConnected(ComponentName name, IBinder service) {
                if (!IabHelper.this.mDisposed) {
                    IabHelper.this.logInfo("Billing service connected.");
    ...
    public void dispose() {
        logInfo("Billing service Disposing.");
        Utils.reportAnalytics("sale", "debug", "purchaseServiceDisposed", null);
        this.mDisposed = true;
        ...
        try {
            ...
            if (!itemType.equals(ITEM_TYPE_SUBS) || this.mSubscriptionsSupported) {
                try {
                    ...
                    Utils.reportAnalytics("sale", "debug", "prePurchase", params);
                    Bundle buyIntentBundle = this.mService.getBuyIntent(BILLING_RESPONSE_RESULT_BILLING_UNAVAILABLE, packageName, sku, itemType, extraData);
                    int response = getResponseCodeFromBundle(buyIntentBundle);
                    if (response != 0) {
                        logError("Unable to buy item, Error response: " + getResponseDesc(response));
                        flagEndAsync();
                        iabResult = new IabResult(response, "Unable to buy item");
                        if (listener != null) {
            ...
            String msg = "Failed to launchPurchaseFlow. " + e3.getMessage();
            reportAsyncFuncError(msg);
            logError(msg);
            iabResult = new IabResult(IABHELPER_UNKNOWN_ERROR, msg);
            if (listener != null) {
                listener.onIabPurchaseFinished(iabResult, null);
            }
        }
    ...
    public boolean handleActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (resultCode == -1 && responseCode == 0) {
            ...
            try {
                ArrayMap<String, String> params = new ArrayMap();
                params.put("purchaseDataLength", Integer.toString(purchaseData.length()));
                params.put("signatureLength", Integer.toString(dataSignature.length()));
                params.put("signature", dataSignature);
                params.put("purchaseData", purchaseData);
                try {
                    params.put("signature64Length", Integer.toString(Base64.decode(dataSignature).length));
                } catch (Base64DecoderException e2) {
                    params.put("errorBase64", ServerProtocol.DIALOG_RETURN_SCOPES_TRUE);
                }
                Utils.reportAnalytics("sale", "debug", "javaPreVerify", params);
                Purchase purchase = new Purchase(this.mPurchasingItemType, purchaseData, dataSignature);
                try {
                    String sku = purchase.getSku();
                    if ("android.test.purchased".equals(sku) || Security.verifyPurchase(this.mSignatureBase64, purchaseData, dataSignature)) {
    ...
    public void queryInventoryAsync(boolean querySkuDetails, List<String> moreSkus, QueryInventoryFinishedListener listener) {
        ...
        try {
            ...
            String msg = "Failed to queryInventoryAsync. " + e.getMessage();
            reportAsyncFuncError(msg);
            logError(msg);
            IabResult result = new IabResult(IABHELPER_UNKNOWN_ERROR, msg);
            if (listener != null) {
        ...
        do {
            ...
            if (response != 0) {
                ...
                for (int i = BILLING_RESPONSE_RESULT_OK; i < purchaseDataList.size(); i += BILLING_RESPONSE_RESULT_USER_CANCELED) {
                    ...
                    if ("android.test.purchased".equals(sku) || Security.verifyPurchase(this.mSignatureBase64, purchaseData, signature)) {
    ...
    int querySkuDetails(String itemType, Inventory inv, List<String> moreSkus) throws RemoteException, JSONException {
        ...
        if (skuDetails.containsKey(RESPONSE_GET_SKU_DETAILS_LIST)) {
            i$ = skuDetails.getStringArrayList(RESPONSE_GET_SKU_DETAILS_LIST).iterator();
            while (i$.hasNext()) {
    ...
    void consumeAsyncInternal(List<Purchase> purchases, OnConsumeFinishedListener singleListener, OnConsumeMultiFinishedListener multiListener) {
        try {
            ...
            String msg = "Failed to consumeAsyncInternal. " + e.getMessage();
            reportAsyncFuncError(msg);
            logError(msg);
            IabResult result = new IabResult(IABHELPER_UNKNOWN_ERROR, msg);
            if (!(this.mDisposed || singleListener == null)) {
                singleListener.onConsumeFinished((Purchase) purchases.get(BILLING_RESPONSE_RESULT_OK), result);
            }
            if (!this.mDisposed && multiListener != null) {
    ...
    void reportAsyncFuncError(String msg) {
        msg = "\"" + msg + "\"";
        ArrayMap<String, String> params = new ArrayMap();
        params.put("msg", msg);
        Utils.reportAnalytics("sale", "debug", "asyncError", params);
    }
    ...
    void logInfo(String msg) {
        Log.i(this.mDebugTag, "In-app billing warning: " + msg);
    }
