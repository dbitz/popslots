...
public class Facebook {
    ...
    String logoutImpl(Context context) throws MalformedURLException, IOException {
        checkUserSession(SFSEvent.LOGOUT);
        Bundle b = new Bundle();
    ...
    public void dialog(Context context, String action, Bundle parameters, DialogListener listener) {
        ...
        if (action.equals(LOGIN)) {
            parameters.putString(MessageColumns.TYPE, "user_agent");
            parameters.putString(ServerProtocol.DIALOG_PARAM_CLIENT_ID, this.mAppId);
        } else {
    ...
    public void setAccessExpiresIn(String expiresInSecsFromNow) {
        ...
        if (expiresInSecsFromNow != null) {
            setAccessExpires(expiresInSecsFromNow.equals(HSConsts.STATUS_NEW) ? 0 : System.currentTimeMillis() + (Long.parseLong(expiresInSecsFromNow) * 1000));
        }
    }
