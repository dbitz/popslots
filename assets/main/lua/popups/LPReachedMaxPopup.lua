...
function Module:init(initParamsTable)
    ...
    log("sceneId=" .. self.scene:id())
	self.scene:addEventListener("closeBtn", self, self.onClose)
	self.scene:addEventListener("connectToFB", self, self.onConnectToFB)
...
function Module:onConnectToFB(eventParamsTable)
    log("onConnectToFB")
    --Events.dispatch("INVOKE_FB_CONNECT_EVENT")
    Utils.connectToFB()
    closePopup(_module.this)
end
