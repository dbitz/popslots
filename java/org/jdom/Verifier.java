...
public final class Verifier {
    ...
    public static String checkCharacterData(String text) {
        ...
        while (i < len) {
            ...
            if (ch >= 55296 && ch <= 56319) {
                ...
                ch = (NativeProtocol.MESSAGE_GET_ACCESS_TOKEN_REQUEST + ((ch - 55296) * 1024)) + (low - 56320);
            }
            if (!isXMLCharacter(ch)) {
                return new StringBuffer().append("0x").append(Integer.toHexString(ch)).append(" is not a legal XML character").toString();
            }
            i++;
        }
        return null;
    ...
    public static String checkNamespacePrefix(String prefix) {
        ...
        for (int i = 0; i < len; i++) {
            ...
            if (!isXMLNameCharacter(c)) {
                return new StringBuffer().append("Namespace prefixes cannot contain the character \"").append(c).append("\"").toString();
            }
        }
        if (prefix.indexOf(":") != -1) {
    ...
    public static String checkNamespaceCollision(Namespace namespace, Namespace other) {
        ...
        return new StringBuffer().append("The namespace prefix \"").append(p1).append("\" collides").toString();
    }
    ...
    public static String checkNamespaceCollision(Namespace namespace, Element element) {
        ...
        if (reason != null) {
            return new StringBuffer().append(reason).append(" with the element namespace prefix").toString();
        }
    ...
    public static String checkNamespaceCollision(Namespace namespace, Attribute attribute) {
        ...
        if (reason != null) {
            return new StringBuffer().append(reason).append(" with an attribute namespace prefix on the element").toString();
        }
        return reason;
    }
    ...
    public static String checkNamespaceCollision(Namespace namespace, List list) {
        ...
        while (reason == null && i.hasNext()) {
            ...
            if (obj instanceof Attribute) {
                ...
                if (reason != null) {
                    reason = new StringBuffer().append(reason).append(" with an additional namespace declared by the element").toString();
                }
            }
        }
        return reason;
    ...
    public static boolean isXMLPublicIDCharacter(char c) {
        ...
        if ((c >= '\'' && c <= DefaultObjectDumpFormatter.TOKEN_DIVIDER) || c == ' ' || c == '!' || c == '=' || c == '#' || c == '$' || c == '_' || c == '%' || c == '\n' || c == '\r' || c == '\t') {
            return true;
        }
        return false;
    }
    ...
    public static String checkPublicID(String publicID) {
        ...
        for (int i = 0; i < publicID.length(); i++) {
            ...
            if (!isXMLPublicIDCharacter(c)) {
                reason = new StringBuffer().append(c).append(" is not a legal character in public IDs").toString();
                break;
            }
        }
        return reason;
    ...
    public static String checkXMLName(String name) {
        ...
        if (!isXMLNameStartCharacter(first)) {
            return new StringBuffer().append("XML names cannot begin with the character \"").append(first).append("\"").toString();
        }
        int len = name.length();
        for (int i = 1; i < len; i++) {
            char c = name.charAt(i);
            if (!isXMLNameCharacter(c)) {
                return new StringBuffer().append("XML names cannot contain the character \"").append(c).append("\"").toString();
            }
        }
        return null;
    }
    ...
    public static String checkURI(String uri) {
        ...
        while (i < uri.length()) {
            ...
            if (isURICharacter(test)) {
                ...
                String msgNumber = new StringBuffer().append("0x").append(Integer.toHexString(test)).toString();
                if (test <= '\t') {
                    msgNumber = new StringBuffer().append("0x0").append(Integer.toHexString(test)).toString();
                }
                return new StringBuffer().append("URIs cannot contain ").append(msgNumber).toString();
            }
        }
        return null;
    }
    ...
    public static boolean isURICharacter(char c) {
        ...
        if ((c >= '0' && c <= '9') || c == '/' || c == '-' || c == '.' || c == '?' || c == ':' || c == '@' || c == '&' || c == '=' || c == '+' || c == '$' || c == ',' || c == '%' || c == '_' || c == '!' || c == '~' || c == '*' || c == '\'' || c == '(' || c == ')') {
            return true;
        }
        return false;
    }
    ...
    public static boolean isXMLCharacter(int c) {
        ...
        if (c < NativeProtocol.MESSAGE_GET_ACCESS_TOKEN_REQUEST) {
            return false;
        }
        if (c > 1114111) {
            return false;
