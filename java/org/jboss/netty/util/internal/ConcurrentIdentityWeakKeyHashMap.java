...
public final class ConcurrentIdentityWeakKeyHashMap<K, V> extends AbstractMap<K, V> implements ConcurrentMap<K, V> {
    ...
    abstract class HashIterator {
        ...
        final void advance() {
            ...
            while (this.nextSegmentIndex >= 0) {
                Segment[] segmentArr = ConcurrentIdentityWeakKeyHashMap.this.segments;
        ...
        boolean containsValue(Object value) {
            if (this.count != 0) {
                for (HashEntry<K, V> e : this.table) {
                    for (HashEntry<K, V> e2 = tab[i]; e2 != null; e2 = e2.next) {
                        V v;
    ...
    private static int hash(int h) {
        ...
        h += h << 3;
        h ^= h >>> 6;
        h += (h << RETRIES_BEFORE_LOCK) + (h << 14);
        return (h >>> DEFAULT_INITIAL_CAPACITY) ^ h;
    }
    ...
    public ConcurrentIdentityWeakKeyHashMap(int initialCapacity, float loadFactor) {
        this(initialCapacity, loadFactor, DEFAULT_INITIAL_CAPACITY);
    }
    ...
    public ConcurrentIdentityWeakKeyHashMap(Map<? extends K, ? extends V> m) {
        this(Math.max(((int) (((float) m.size()) / DEFAULT_LOAD_FACTOR)) + 1, DEFAULT_INITIAL_CAPACITY), DEFAULT_LOAD_FACTOR, DEFAULT_INITIAL_CAPACITY);
        putAll(m);
    }
    ...
    public int size() {
        ...
        if (check != sum) {
            sum = 0;
            for (Segment lock : segments) {
                lock.lock();
            }
            for (Segment lock2 : segments) {
                sum += (long) lock2.count;
            }
            for (Segment lock22 : segments) {
                lock22.unlock();
            }
        }
        if (sum > 2147483647L) {
    ...
    public void purgeStaleEntries() {
        for (Segment removeStale : this.segments) {
